    ...
    while (getline(inFile, line))
    {
        ...
        if (line == "GATES")
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (s_type == "NOT")
                {
                    std::string s_in1;          // decl to hold in stuff
                    getline(ss, s_in1, ',');    // take in in wires
                    std::string s_output;       // decl to hold out stuff
                    getline(ss, s_output, ','); // take in out wires

                    int inputId = stoi(s_in1);            // conv to int
                    int outputId = stoi(s_output);        // conv to int
                    Wire *inputWire = m_wires[inputId];   // get the ptr to in wire from vectr
                    Wire *outputWire = m_wires[outputId]; // get the ptr to out wire from vct

                    m_gates.push_back(new NotGate(inputWire, outputWire)); // add new notgate to vect of gates
                }
            }
        }
        if (line == "INJECT")
